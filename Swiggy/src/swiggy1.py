# -*- coding: utf-8 -*-
"""
Created on Thu Dec 26 13:23:12 2019
...
#    Branch.index()
        ...
        for index1 , pincode in tqdm(enumerate(Branch[int(start_ind):15])):
        #    pincode = Branch[index1] index1 = 4
            try:
                driver.refresh()
                try:
                    f=open(r'D:\Hdfc_Scrapping_data\Uday\Scrapping_data\Swiggy\index.text','w+',encoding='utf-8')
                ...
                suggested_clicked_ele = wait.until(EC.presence_of_element_located((By.XPATH,'//*[@id="root"]/div[1]/header/div/div/div/span[1]/span'))).get_attribute('innerText')
                ...
                cols=['Restaurant name','rating','Type_of_food','Area','Delivery_Time','Cost_For_Two','Period','Searched_Area']
                global data
    ...
    #                        resturants = ['Shiv Sagar ( Authentic Pav Bhaji )', 'South Indian, Snacks, Beverages', '--', '•', '38 MINS', '•', '₹1 FOR TWO']
                            ...
                            data = data.append({'Restaurant name':resturants[0],'rating': resturants[2] if resturants[2] != '--' else None  ,'Type_of_food':resturants[1],'Area':pincode,'Delivery_Time':int(re.findall('\d{2,4}',resturants[4])[0]),'Cost_For_Two':int(re.findall('\d{1,4}',resturants[6])[0]),'Period':period,'Searched_Area':suggested_clicked_ele},ignore_index=True)
                            time.sleep(1)
                ...
                print('no data')
                print(pincode)
                data_main = data_main.append({'Restaurant name':None,'rating':None ,'Type_of_food':None,'Area':pincode,'Delivery_Time':None,'Cost_For_Two':None,'Period':period,'Searched_Area':suggested_clicked_ele},ignore_index = True)
