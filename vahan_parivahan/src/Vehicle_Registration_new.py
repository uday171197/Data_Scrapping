...
while T == True:
    try:
        ...
        try:
            ...
            f.write(start_year)
            f.close()
        for year in range(int(start_year),2019):
            #year=2018
            try:
                f=open(r'D:\Hdfc_Scrapping_data\Uday\Scrapping_data\vahan_parivahan\Vehgical_registration\index.text','w+',encoding='utf-8')
            except:
            ...
            for month in range(int(start_month),13):
                #for upto   month=1
                Period = Ending_date(wait,month,driver,year)


#            #for from
                Rdate = Starting_date(wait,month,driver,year)
                print(Rdate)
...
#                                print(table_data.shape) classwise_data.columns
